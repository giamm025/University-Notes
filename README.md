# 🎓 University Notes

Ciaoo! In questa repo puoi trovare tutti gli appunti, codici, esercizi e progetti sviluppati durante tutto il percorso di studi in Informatica (triennale) e Computer Science (magistrale). 

---

## 📂 Struttura della Repository e Corsi

Naviga direttamente nelle cartelle dei corsi cliccando sui link qui sotto:

### 📗 1° Anno
* **1° Semestre**
  * 💻 [Fondamenti di Programmazione](./1-Anno/1-Semestre/Fondamenti-di-Programazione)
  * 📐 [Metodi Matematici per l'Informatica](./1-Anno/1-Semestre/Metodi-Matematici-per-l'Informatica)
  * 🧮 [Sistemi Digitali](./1-Anno/1-Semestre/Sistemi-Digitali)
* **2° Semestre**
  * ⚙️ [Architettura degli Elaboratori](./1-Anno/2-Semestre/Architettura-degli-Elaboratori)
  * 💡 [Introduzione agli Algoritmi](./1-Anno/2-Semestre/Introduzione-agli-Algoritmi)
  * 🖥️ [Metodologie di Programmazione](./1-Anno/2-Semestre/Metodologie-di-Programmazione)
    * 🏆 **Progetto:** [JPokeBattle](./1-Anno/2-Semestre/Metodologie-di-Programmazione/Progetto/JPokeBattle-Consegnato)

### 📘 2° Anno
* **1° Semestre**
  * 🧮 [Algebra](./2-Anno/1-Semestre/Algebra)
  * 🗄️ [Basi di Dati 1](./2-Anno/1-Semestre/Basi-di-Dati-1)
  * 🎲 [Calcolo delle Probabilità](./2-Anno/1-Semestre/Calcolo-delle-Probabilità)
  * 🖥️ [Sistemi Operativi 1](./2-Anno/1-Semestre/Sistemi-Operativi-1)

* **2° Semestre**
  * 💡 [Progettazione di Algoritmi](./2-Anno/2-Semestre/Algoritmi-2)
  * 🗄️ [Basi di Dati 2](./2-Anno/2-Semestre/Basi-di-Dati-2)
  * 🌐 [Reti degli Elaboratori](./2-Anno/2-Semestre/Reti-degli-Elaboratori)
  * 🖥️ [Sistemi Operativi 2](./2-Anno/2-Semestre/Sistemi-Operativi-2)
      * 🏆 **Homework 1:** [C Pre-Compiler](https://github.com/MaxTheOne05/homework1.git)
      * 🏆 **Homework 2:** [Parallel Client-Server XOR Cipher](https://github.com/MaxTheOne05/homework2.git)

### 📙 3° Anno
* **1° Semestre**
  * 🧮 [Automi Calcolabilità e Complessità](./3-Anno/1-Semestre/Automi-Calcolabilita-Complessita)
  * 🖥️ [Ingegneria del Software](./3-Anno/1-Semestre/ISW)
  * 🛡️ [Cybersecurity](./3-Anno/1-Semestre/Cyber-Security)
  * 🖱️ [Interazione Uomo-Macchina](./3-Anno/1-Semestre/IUM)
  * 🕸️ [Programmazione per il Web](./3-Anno/1-Semestre/WEB)
      * 🏆 **Progetto:** [WasaText](https://github.com/giamm025/wasaText.git)
* **2° Semestre**
  * 🏢 [Organizzazione e Gestione Aziendale](./3-Anno/2-Semestre/OGA)
  * 🤖 [Ai Lab: Computer Vision and NLP](./3-Anno/2-Semestre/AI-LAB)
      * 🏆 **Progetto:** [Sign Language Recognition (LSTM + MediaPipe)](https://github.com/giamm025/Ai-Lab-Project.git)

* **Tirocinio & Tesi**
  * 🖥️ [iBEARer](https://github.com/giamm025/iBEARer.git)
  * 📜 [Tesi in pdf](./3-Anno/Tesi.pdf) 

---

## 🛠️ Tecnologie & Strumenti Utilizzati

- **Linguaggi:** `Python`, `Java`, `Javascript`, `C/C++`, `SQL`
- **Database:** `PostgreSQL`, `pgAdmin`, `SQLite`
- **Tools & Paradigmi:** `Git`, `Docker`, `UML`, Object-Oriented Programming (OOP)

---

## 📌 Note per chi esplora
Tutto il materiale in questa repository è reativo ed è stato utilizzo per il superamento degli esami durante il MIO percorso di studi. A causa dei recenti cambiamenti del regolamento, dei docenti e dei corsi erogati nella laurea triennale in Informatica NON garantisco che gli argomenti coincidano o siano sufficienti per le  nuove modalità di esame. Inoltre, potrebbe esserci materiale incompleto, bug o errori nelle dispense. Se doveste trovarne potete conttatrmi direttamente e cercherò di correggere!

---
👨‍💻 **Autore:** Francesco Giammaria (`giamm025` & `giamm025-Uni`)
